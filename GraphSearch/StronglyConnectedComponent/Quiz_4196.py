...
def dfs(start):
    ...
    visited[start] = count
    par = visited[start]
    for nxt in graph[start]: ## 현재 1 // 다음 노드 : 4,6 
        if visited[nxt]==-1:
            par = min(par, dfs(nxt))
        elif SCC[nxt]==-1: ## 방문했지만, 아직 SCC 가 아닌 경우.
            ## 현재 5번 노드, next 1번 노드 //
            ## visited[1] = 1 // par=visited[5] =3
            par = min(par, visited[nxt]) ## visited[nxt] => 방문한적있는 노드 그 때, 그 노드의 방문 순서
