...
Permutation => 8! = 40320
50이닝 진행 
따라서 Brute-force로 해도 충분히 구할 수 있다.
하지만 Python으로는 무리 => Pypy3 로 정답
"""

import sys
from itertools import permutations

def Game(orders, pos_score):
    score = 0
    now_hitman = 0
    ...
    for i in range(len(pos_score)):
        ...
        out_count = 0
        while out_count<3: 
            hitman = orders[now_hitman]
            if pos_score[i][hitman] == 0:
                out_count += 1
            elif pos_score[i][hitman] == 1:
                score += b3
                b1,b2,b3 = 1, b1, b2
            elif pos_score[i][hitman] == 2:
                score += (b2+b3)
                b1,b2,b3 = 0,1,b1
            elif pos_score[i][hitman] == 3:
                score += (b1+b2+b3)
                b1,b2,b3 = 0, 0, 1
            elif pos_score[i][hitman] == 4:
                score += (b1+b2+b3+1)
                b1,b2,b3 = 0,0,0
            
            now_hitman = (now_hitman+1)%9
    return score

input = sys.stdin.readline
n = int(input())

pos_score = [list(map(int, input().split())) for _ in range(n)]

p = permutations([i for i in range(1, 9)])
max_value = float('-Inf')
flag = True
for order_hitman in p:
    order_hitman = list(order_hitman[:3]) + [0] + list(order_hitman[3:])
    max_value = max(max_value, Game(orders=order_hitman, pos_score=pos_score))
